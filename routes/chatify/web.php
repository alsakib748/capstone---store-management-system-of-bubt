...
// Backward-compatible route name used across the app (e.g. sidebar links)
Route::get('/chatify', function () {
    return redirect()->route(config('chatify.routes.prefix'));
})->middleware(['auth'])->name('chatify');

/*
 * Chatify routes are grouped under the `chat` prefix and protected
 * by the `auth` middleware to prevent unauthenticated access.
 */
Route::prefix('chat')->middleware(['auth'])->group(function () {
     ...
     * NOTE: use '/auth' here so the final URL is '/chat/auth'
     */
    Route::post('/auth', [MessagesController::class, 'pusherAuth'])->name('pusher.auth');
