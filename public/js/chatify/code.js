 ...
 *-------------------------------------------------------------
 */
function getRoleBasedUsers() {
    $.ajax({
        url: url + "/getRoleBasedUsers",
        method: "GET",
        data: { _token: csrfToken, page: 1 },
        dataType: "JSON",
        success: (data) => {
            console.log("=== ROLE USERS DEBUG ===");
            console.log("Raw data:", data);
            console.log("Type:", typeof data);
            console.log("data.contacts type:", typeof data.contacts);
            console.log("data.contacts value:", data.contacts);
            console.log("========================");

            // Check if contacts property exists (even if empty string)
            if (data && data.hasOwnProperty("contacts")) {
                $(".roleBasedUsers").html(data.contacts);
                console.log("✓ Role-based users loaded");
                // update data-action required with [responsive design]
                cssMediaQueries();
            } else {
                console.warn("✗ No contacts property in response");
                $(".roleBasedUsers").html(
                    '<p class="message-hint center-el"><span>No staff users available</span></p>',
                );
            }
        },
        error: (xhr, status, error) => {
            console.error("✗ Error loading role-based users:", error);
            $(".roleBasedUsers").html(
                '<p class="message-hint center-el"><span>Error loading staff users</span></p>',
            );
        },
    });
}
