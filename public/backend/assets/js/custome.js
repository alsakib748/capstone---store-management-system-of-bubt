document.addEventListener("DOMContentLoaded", function () {
  ...
  `;
    ...
    let inputDiscountEl = document.getElementById("inputDiscount");
    let inputShippingEl = document.getElementById("inputShipping");
    let paidAmountEl = document.querySelector("input[name='paid_amount']");
    let fullPaidEl = document.querySelector("input[name='full_paid']");

    if (inputDiscountEl)
        inputDiscountEl.addEventListener("input", updateGrandTotal);
    if (inputShippingEl)
        inputShippingEl.addEventListener("input", updateGrandTotal);
    if (paidAmountEl) paidAmountEl.addEventListener("input", updateDueAmount);
    // new add full paid functionality
    if (fullPaidEl) fullPaidEl.addEventListener("input", updateDueAmount);
  ...
  `;
    ...
    document
        ...
        .addEventListener("click", function () {
            let updatedPrice = parseFloat(
                document.getElementById("modalPrice").value.replace("TK ", ""),
            );
            let discountValue =
    ...
    let displayDiscountEl = document.getElementById("displayDiscount");
    let shippingDisplayEl = document.getElementById("shippingDisplay");

    if (inputDiscountEl && displayDiscountEl) {
        inputDiscountEl.addEventListener("input", function () {
            displayDiscountEl.textContent = this.value || 0;
        });
    }
    if (inputShippingEl && shippingDisplayEl) {
        inputShippingEl.addEventListener("input", function () {
            shippingDisplayEl.textContent = this.value || 0;
        });
    }
});
